...
import { useTransliterator } from "@/hooks/useTransliterator";

const TypingInfoPage: React.FC = () => {
  const { wasmReady, schemas } = useTransliterator();
  const [selectedLanguage, setSelectedLanguage] = useState("as");

  if (!wasmReady || !schemas.assamese) {
    return (
  ...
  const assameseSchema = schemas.assamese;
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <h1 className="text-3xl font-bold mb-6">
        How to Type {languages.find(l => l.code === selectedLanguage)?.name} Characters
      </h1>
      {renderInstructions()}
